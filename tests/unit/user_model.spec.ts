...
test.group('User model | hachage automatique du mot de passe', (group) => {
  ...
  test('updateOrCreate() hache le mot de passe sur la branche mise à jour', async ({ assert }) => {
    ...
    // La ligne existante est réutilisée : le hook `beforeSave` doit s'appliquer
    // aussi sur ce chemin, pas seulement à la création.
  ...
  test('verifyCredentials rejette un mot de passe vide', async ({ assert }) => {
    await User.create({ email: 'hedy@example.com', password: 'secret-password' })

    await assert.rejects(() => User.verifyCredentials('hedy@example.com', ''))
  })
